...
export default {
  async fetch(request,env){
    ...
    if(url.pathname==='/healthz')return json({ok:true,service:'am-digital-lab-public',version:SITE_VERSION,crm:Boolean(env.CRM_SERVICE)});

    if(url.pathname==='/api/start-project'&&request.method==='POST'){
      try{
        const body=await request.text();
        if(body.length>16000)return json({error:'Request too large.'},413);
        const upstream=await env.CRM_SERVICE.fetch('https://crm.internal/api/public/leads',{
          method:'POST',
        ...
        console.error('Project inquiry service binding failed',err);
