...
const DATA = process.env.AMDL_DATA_DIR || path.join(ROOT, 'data');
fs.mkdirSync(DATA, { recursive: true });
const DB_PATH = process.env.AMDL_DB_PATH || path.join(DATA, 'amdl-crm.sqlite');
...
const adminEmail = process.env.AMDL_ADMIN_EMAIL || 'admin@amdigital.local';
const adminPassword = process.env.AMDL_ADMIN_PASSWORD || (isProduction ? '' : 'change-me-123');
if (!db.prepare('SELECT COUNT(*) c FROM users').get().c) {
  if (!adminPassword || adminPassword.length < 12) {
    throw new Error('AMDL_ADMIN_PASSWORD wajib diisi minimal 12 karakter sebelum production deployment.');
  }
  db.prepare('INSERT INTO users (name,email,password_hash,role) VALUES (?,?,?,?)').run(
    'AM DIGITAL LAB Owner',adminEmail,hashPassword(adminPassword),'OWNER'
  );
}
...
module.exports={db,DB_PATH,verifyPassword,today,nextCode,log,activeProjectAllowed,scopeReady,hasBlockingQc};
